...
define('condition', function () {
    return function ($scope) {
        ...
        $scope.onTimeSlotChange = function () {
            ...
            if (slot) {
                $scope.customTimeSlot.endTime = dateUtil.format(calcTime(slot, getSelTime()), 'yyyy-MM-dd hh:mm');
            }
        };
        //获取当前选择的时间
        function getSelTime() {
        ...
        function resetHours() {
            if ($scope.customTimeSlot.startTime >= dateUtil.format(new Date(), 'yyyy-MM-dd')) { //最近一天
                ...
                $scope.sliderRefresh = !$scope.sliderRefresh;
        ...
        function reShowSlots() {
            ...
            var index = _.findIndex($scope.timeSlots, function (slot) {
                return calcTime(slot, selTime) > endTime;
            });
         ...
         */
        function calcTime(timeSlot, startTime) {
            ...
            switch (timeSlot.unit) {
                case 'MIN':
                    return dateUtil.addInteger(endTime, dateUtil.calendar.minute, (timeSlot.value - 0) * calc);
                case 'HOUR':
                    return dateUtil.addInteger(endTime, dateUtil.calendar.hour, (timeSlot.value - 0) * calc);
                case 'DAY':
                    return dateUtil.addInteger(endTime, dateUtil.calendar.day, (timeSlot.value - 0) * calc);
            }
        }
         ...
         */
        $scope.changeTimeSlot = function (timeSlot, start_time) {
            $scope.customTimeSlotActive = false;
            var endTime = null, startTime = null;
            if (start_time) {
                startTime = start_time;
                endTime = calcTime(timeSlot, startTime);
            } else {
                endTime = new Date();
                startTime = calcTime(timeSlot);
            }
            //改变条件
            changeCondition({
                unit: timeSlot.unit,
        ...
        $scope.changeCustomTimeSlot = function () {
            ...
            $scope.changeTimeSlot(slot, start_time);
